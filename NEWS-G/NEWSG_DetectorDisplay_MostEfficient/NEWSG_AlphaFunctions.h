...
Alpha createNewPo210Alpha() {
  ...
  if (quadrantNumber ==  1) {
    for (int loc = 0; loc < sizeof (xMPPFQ1Po210[0]) / sizeof (xMPPFQ1Po210[0][0]); loc++) {
...
Alpha createNewRn222Alpha() {
  ...
  if (quadrantNumber ==  1) {
    for (int loc = 0; loc < sizeof (xMPPFQ1Rn222[0]) / sizeof (xMPPFQ1Rn222[0][0]); loc++) {
      newRn222Alpha.xMovementPath[loc] = xMPPFQ1Rn222[pathNumber][loc]; // Fills the movement arrays.
      newRn222Alpha.yMovementPath[loc] = yMPPFQ1Rn222[pathNumber][loc];
    }
    newRn222Alpha.energy = Rn222alphaEnergy;
    newRn222Alpha.movementLocation = 0; // Creates the new cosmic ray with the new values.
  }
  else if (quadrantNumber ==  2) {
    for (int loc = 0; loc < sizeof (xMPPFQ1Rn222[0]) / sizeof (xMPPFQ1Rn222[0][0]); loc++) {
      newRn222Alpha.xMovementPath[loc] = xMPPFQ1Rn222[pathNumber][loc]; // Fills the movement arrays.
...
Alpha computeNextPo210AlphaLocation(Alpha Po210Alpha) {
  ...
  if (Po210Alpha.movementLocation < sizeof(Po210Alpha.xMovementPath) / sizeof(Po210Alpha.xMovementPath[0])) { // If the alpha can exist.
...
Alpha computeNextRn222AlphaLocation(Alpha Rn222Alpha) {
  ...
  if (Rn222Alpha.movementLocation < sizeof (Rn222Alpha.xMovementPath) / sizeof (Rn222Alpha.xMovementPath[0])) { // If the alpha can exist.
    int energyChange = random(energyChangeLowerBounds, energyChangeUpperBounds); // Calculates the energy change that the electron will gain if created in keV.
    alphaElectrons[alphaElectronLocation] = generateElectron(xCurrent, yCurrent, energyChange); // Generates an electron and saves it.
    energyCurrent = energyCurrent - energyChange; // Lowers the energy of the alpha.
    alphaElectronLocation++; // Increases the count for which the next electron will be added into the array.
