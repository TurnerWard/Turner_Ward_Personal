...
// Creates an PointLikeParticle struct. A PointLikeParticle is made up of a radial location, and angular location, and an energy.
typedef struct PointLikeParticle {
...
PointLikeParticle pointlikeparticle1;
...
bool checkPointLikeParticleElectronDist() {
  for (int loc = 0; loc < numElectronLocationsInArray; loc++) { // For each variable in the electron array.
    if (pointlikeparticlesElectrons[loc].rLocation < 2048) // Check to see if the radial location is greater then 20.
...
PointLikeParticle createNewPointLikeParticle() {
  ...
  Serial.print("Point Like Particle -> Path #: "); Serial.print(pathNumber); Serial.print("\t Quadrant #: "); Serial.println(quadrantNumber);
  ...
  if (quadrantNumber ==  1) { // If in quadrant 1.
    ...
    for (int loc = 0; loc < numElectronLocationsInArray; loc++) { // For each step for the particle.
      newPointLikeParticle.xMovementPath[loc] = -1 * pgm_read_word(&(xMPPFQ1PointLikeParticle[pathNumber][loc])); // Fills the x movement array from PROGMEM.
      newPointLikeParticle.yMovementPath[loc] = pgm_read_word(&(yMPPFQ1PointLikeParticle[pathNumber][loc])); // Fills the y movement array from PROGMEM.
    }
    newPointLikeParticle.energy = pgm_read_word(&(PointLikeParticleEnergy[pathNumber])); // Enters the cosmic rays energy.
    newPointLikeParticle.movementLocation = arrayStartLocation; // Sets the arrays start location.
    ...
    for (int loc = 0; loc < numElectronLocationsInArray; loc++) { // For each step for the particle.
      newPointLikeParticle.xMovementPath[loc] = -1 * pgm_read_word(&(xMPPFQ1PointLikeParticle[pathNumber][loc])); // Fills the x movement array from PROGMEM.
      newPointLikeParticle.yMovementPath[loc] = -1 * pgm_read_word(&(yMPPFQ1PointLikeParticle[pathNumber][loc])); // Fills the y movement array from PROGMEM.
    }
    newPointLikeParticle.energy = pgm_read_word(&(PointLikeParticleEnergy[pathNumber])); // Enters the cosmic rays energy.
    newPointLikeParticle.movementLocation = arrayStartLocation; // Sets the arrays start location.
  }
